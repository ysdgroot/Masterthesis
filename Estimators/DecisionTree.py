...
########################################################
criterion = ["mse", "mae"]
min_samples_leaf = 1    # zou hier voor een random fractie kiezen van de samples
max_features = ["auto", "sqrt", "log2"]     # Er zijn niet zoveel features, dus mss best nog op default laten
...
start = time.time()
model_decisiontree = DecisionTreeRegressor().fit(X, y)
end = time.time()
...
pred_y = model_decisiontree.predict(X_test)


score = model_decisiontree.score(X_test, y_test)
print(score)
