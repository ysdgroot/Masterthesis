...
file_name = 'ParallelTest-steps and accuracy-BS-v3.csv'
maturity = 10
interest_rate = 0.001
volatitlity = 0.1
start_price = 100
...
def start_iteration_function(time_step):
...
for amount in amount_paths:
    Parallel(n_jobs=3)(delayed(start_iteration_function)(time_step) for time_step in time_steps_per_maturities)
    ...
    #     rel_diff = (approx_call - exact_call) / exact_call
    #
    #     temp_result = [time_step, amount, time, rel_diff_abs, rel_diff, exact_call]
    #     with open(file_name, 'a', newline='') as fd:
    #         writer = csv.writer(fd)
    #         writer.writerow(temp_result)
